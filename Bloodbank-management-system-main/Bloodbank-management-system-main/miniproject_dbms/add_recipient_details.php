...
if (isset($_REQUEST['reci_name'])) {
    ...
    $has_bgrp = mysqli_query($con, "SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='" . DB_NAME . "' AND TABLE_NAME='recipient' AND COLUMN_NAME='reci_bgrp' LIMIT 1");
    $use_bgrp = $has_bgrp && mysqli_num_rows($has_bgrp) > 0;

    if ($use_bgrp) {
        $query    = "INSERT into `recipient` (reci_name, reci_age, reci_sex, reci_phno, reci_bgrp, reci_reg_date)
                         VALUES ('$reci_name', '$reci_age', '$reci_sex', '$reci_phno', NULL, current_timestamp());";
    } else {
        $query    = "INSERT into `recipient` (reci_name, reci_age, reci_sex, reci_phno, reci_reg_date)
                         VALUES ('$reci_name', '$reci_age', '$reci_sex', '$reci_phno', current_timestamp());";
    }
